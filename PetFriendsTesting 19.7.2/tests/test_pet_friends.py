...
def test_add_new_create_pet_simple_with_valid_data(name ='Барсик', animal_type ='дворянин',
                                     age ='6'):
...
def test_add_new_pets_set_photo_pet_id_with_valid_data(pet_photo = 'images/P1040103.jpg'):
    """Проверяем что можно добавить фото питомца """

    # Получаем полный путь изображения питомца и сохраняем в переменную pet_photo
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)

    # Запрашиваем ключ api и сохраняем в переменую auth_key и список моих питомцев
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    ...
    if len(my_pets['pets']) > 0:
        status, result = pf.add_new_pets_set_photo_pet_id(auth_key, my_pets['pets'][0]['id'], pet_photo)[0]
...
def test_add_new_pet(name ='//////', animal_type ='******',
                                     age ='Four', pet_photo ='images/cat1.jpg'):
...
def test_successful_delete_pet():
    """Проверяем возможность удаления не существующего питомца"""
    # Получаем ключ auth_key и запрашиваем список своих питомцев
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
